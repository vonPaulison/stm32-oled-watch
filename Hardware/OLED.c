...
/**
  * 函    数：OLED设置亮度 (对比度)
  * 参    数：Brightness 亮度值, 范围0~255 (0最暗, 255最亮)
  * 返 回 值：无
  * 说    明：通过SSD1306的0x81命令设置对比度寄存器
  *           OLED_Init中默认设为0xCF, 调用本函数立即生效
  *           断电不保存, 需要持久化请在应用层用Flash/BKP寄存器存储
  */
void OLED_SetBrightness(uint8_t Brightness)
{
	OLED_WriteCommand(0x81);			//对比度命令
	OLED_WriteCommand(Brightness);		//对比度值, 0x00~0xFF
}
...
//	X += 2;
	...
	OLED_WriteCommand(0x02 | (X & 0x0F));			//设置X位置低4位
}
